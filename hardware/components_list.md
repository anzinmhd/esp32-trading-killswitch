...
Below is the definitive list of hardware components used to build the Trade-Lab ESP32 Kill Switch (Phase 2).

## Core Components

| Component | Description / Specification | Quantity | Notes |
| :--- | :--- | :--- | :--- |
| **Microcontroller** | ESP32 38-pin Dev Board (ESP-WROOM-32) | 1 | Must have CP2102 USB-to-UART chip. Required for the Phase 2 WiFi integration and embedded Web Server. |
| **Display** | SSD1306 0.96" OLED | 1 | I2C interface (4 pins: VCC, GND, SCL, SDA). 128x64 resolution. |
| **Switch** | Lever Switch (or Mushroom Button) | 1 | 2-pin (centre + left/right). Provides tactile feedback for emergency activation. |
...
| **Prototyping** | Breadboard & Jumper Wires | 1 set | For assembling the circuit without soldering during the paper trading phase. |
...
If you are sourcing parts for a similar build, keep the following in mind based on our hardware troubleshooting:
...
*   **Active Buzzer (Active LOW logic):** Pay close attention to your buzzer module. While many tutorials assume HIGH = ON, the specific module used here triggers on a LOW signal. We inverted the logic in our C++ firmware to prevent the buzzer from staying on permanently.
